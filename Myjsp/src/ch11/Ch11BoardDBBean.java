...
public class Ch11BoardDBBean {
	...
	//		sql += "ref,re_step,re_level,content,ip) values ((SELECT NVL(MAX(NUM),0) + 1 FROM BOARD) ,?,?,?,?,?,?,?,?,?,?)";
			...
			pstmt3.setString(2, article.getWriter());
			pstmt3.setString(3, article.getEmail());
			pstmt3.setString(4, article.getSubject());
			pstmt3.setString(5, article.getPasswd());
			pstmt3.setTimestamp(6, article.getReg_date());
			pstmt3.setInt(7, ref);
			pstmt3.setInt(8, re_step);
			pstmt3.setInt(9, re_level);
			pstmt3.setString(10, article.getContent());
			pstmt3.setString(11, article.getIp());
...
//board테이블에 저장된 전체글의 수를 얻어냄( select 문 )<= list.jsp에서 사용
...
// 글의 목록(복수개의 글)을 가져옴(select문) <=list.jsp에서 사용
	public List<Ch11BoardDataBean> getArticles(int start, int end) throws Exception {
		...
		try {
			 ...
			 * "   				,RE_STEP ASC	" + "			 ) O					" +
			 * "		)							" +
			 * "WERE RUNM BETWEEN ? AND ? 			");
			 * 
			 * pstmt.setInt(1, start); pstmt.setInt(2, start+end-1);
			 */
...
//글 수정처리에서 사용(update문) <= updatePro.jsp에서 사용
	public int updateArticle(Ch11BoardDataBean article)
