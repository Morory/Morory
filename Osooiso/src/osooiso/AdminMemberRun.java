...
public class AdminMemberRun implements ActionListener, ItemListener, MouseListener{
	...
	//1.
	private String[]	saTit		= new String[] {"코드", "이메일", "비밀번호", "전화번호", "닉네임", "가입일자", "회원 여부" };	//컬럼 만들어주는 것	//string array title
	private int []		iaCwidth	= new int	[] {10,		80,		80,		80,		50,		80,		10};	//칼럼 폭//int array column width
	private int []		iaAlm		= new int 	[]	{JLabel.RIGHT, JLabel.LEFT, JLabel.LEFT,JLabel.CENTER, JLabel.LEFT, JLabel.LEFT, JLabel.CENTER};
...
//	}
	...
	private void initialize() {
		...
		frame = new JFrame();
		frame.setBounds(0, 290, 870, 468);//frame.setBounds(500, 290, 870, 468);
		...
		cmbCodeName.setModel(new DefaultComboBoxModel<String>(new String[] {"코드", "이메일", "회원여부"}));
		cmbCodeName.setBounds(103, 52, 90, 21);
		...
		scrollPane.setViewportView(table);			//테이블을 스크롤 페인에서 보이게 하라
		
		label_2 = new JLabel("닉네임");
		label_2.setFont(new Font("굴림", Font.BOLD, 15));
		label_2.setBounds(554, 328, 51, 15);
		frame.getContentPane().add(label_2);
		
		tfNcnm = new JTextField();
		tfNcnm.setColumns(10);
		tfNcnm.setBounds(608, 325, 116, 21);
		frame.getContentPane().add(tfNcnm);
		
		label_5 = new JLabel("가입일자");
		label_5.setFont(new Font("굴림", Font.BOLD, 15));
		label_5.setBounds(356, 377, 66, 15);
		frame.getContentPane().add(label_5);
		
		label_6 = new JLabel("회원여부");
		label_6.setForeground(Color.RED);
		label_6.setFont(new Font("굴림", Font.BOLD, 15));
		label_6.setBounds(554, 377, 66, 15);
		frame.getContentPane().add(label_6);
		
		tfSrbde = new JTextField();
		tfSrbde.setColumns(10);
		tfSrbde.setBounds(426, 374, 116, 21);
		frame.getContentPane().add(tfSrbde);
		
		tfMberAt = new JTextField();
		tfMberAt.setColumns(10);
		tfMberAt.setBounds(632, 374, 41, 21);
		frame.getContentPane().add(tfMberAt);
		...
		btnBBSCTT = new JButton("게시물관리");
		btnBBSCTT.setFont(new Font("굴림", Font.BOLD, 12));
		btnBBSCTT.setEnabled(true);
		btnBBSCTT.setBounds(736, 306, 106, 31);
		frame.getContentPane().add(btnBBSCTT);
		btnBBSCTT.addActionListener(this);
	...
	public void actionPerformed(ActionEvent e) {
		...
		{	
			...
			{
				JOptionPane.showMessageDialog(frame, "조회조건을  입력하세요.");	//JOptionPane 는 클래스이름 (스태틱) //JOptionPane.showMessageDialog 대화창 여는 메소드// frame < 부모화면 23번 줄.// 부모창이 없으면 null
				return;
			}
		...
		else if(e.getSource() == btnUpdate)	//삭제 버튼
		{		
			update();
		}
		...
		else if(e.getSource() == btnBBSCTT)
		{
			frame.setVisible(false);
			new AdminBbscttRun();
			
//			AdminBbscttRun.class.frame.setVisible(true);
		}
	}
...
//		start = (pageNum -1) * 5 + 1;	//몇번 부터 시작하는지
	...
	public void clear()
	{
		...
		tfEmail.setText("");		//밑에 이메일 텍스트부분 공백으로	
		tfTelno.setText("");		//밑에 전화번호 텍스트부분 공백으로
		tfNcnm.setText("");			//밑에 닉네임 텍스트부분 공백으로
		tfSrbde.setText("");		//밑에 가입일자 텍스트부분 공백으로
		tfMberAt.setText("");		//밑에 회원여부 텍스트부분 공백으로
	}
		
	
	public void update () {	//★★★★★ key 존재 확인해야하고 수정인 경우에는 쿼리하나로 해결하기 위해 하나로 묶는다?? //db에서 읽어올때 select 를 왕창 다 읽어와서 일부분만 써버리자 
		String sSearchSingle = "SELECT * FROM MBER WHERE MBER_NO = ?";
		String sUpdate = "UPDATE MBER SET MBER_AT = ?   WHERE MBER_NO = ?";
		
		int ret;
		
		String strMberNo = tfCode.getText();
		String strMberAt = tfMberAt.getText();
		

		if (strMberNo.isEmpty())
		{
		...
		try {
			...
			pstmt2.setString(1, strMberAt);
			pstmt2.setInt(2, Integer.parseInt(strMberNo));
	...
	public void itemStateChanged(ItemEvent e) {
		if(e.getSource() == cmbCodeName )	//콤보박스에 이벤트가 생겻냐. 라는 뜻
		{
			...
			{	
				btnNext.setEnabled(false);
				if("코드".equals(e.getItem().toString())	)				//어떤항목을 get
				{
					iSearch = CODE;	//isearch 라는 변수에 code 넣
				}
				else if ("이메일".equals(e.getItem()))
				{
					iSearch = EMAIL;
				}
				else if ("회원여부".equals(e.getItem()))
				{
					iSearch = MBER_AT;
				}
				else
...
//			System.out.println(row + "," + col);	// 선택된 위치 값을 출력
			...
			if(row > -1)					// table 에서 특정 행을 클릭한 경우
			{
				...
				tfEmail.setText	((String)table.getValueAt(row, 1));
				tfEmail.setEnabled(false);
				tfTelno.setText	((String)table.getValueAt(row, 3));
				tfTelno.setEnabled(false);
				tfNcnm.setText	((String)table.getValueAt(row, 4));
				tfNcnm.setEnabled(false);
				tfSrbde.setText ((String)table.getValueAt(row, 5));
				tfSrbde.setEnabled(false);
				tfMberAt.setText((String)table.getValueAt(row, 6));
